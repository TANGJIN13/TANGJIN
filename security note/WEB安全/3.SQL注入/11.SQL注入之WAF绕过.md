...
```
    ...
    空格不能使用=%09,%0a,%0b,%0c,%0d,%20,%a0，like等
...
使用HANDLER命令时，例如  HANDLER your_table OPEN  打开表，然后用  HANDLER your_table READ FIRST  等方式读取数据，最后用  HANDLER your_table CLOSE  关闭操作。它可以像游标一样按行处理数据。
[[强网杯 2019随便注]]